...
for (const file of files) {
  ...
  if (!candidate || !images.has(candidate)) {
    const isPolicyEra = typeof data.date === "string" && data.date >= POLICY_EFFECTIVE_DATE;
    if (isPolicyEra) {
      console.error(`X ${file}: artículo ${data.status} sin imagen asociada en public/images`);
      errors += 1;
    } else {
      console.warn(`⚠ ${file}: artículo histórico ${data.status} sin imagen asociada; se mantiene como aviso de legado`);
      warnings += 1;
    }
  }
}
...
if (errors) {
  console.error(`Fallo: ${errors} artículo(s) de la política vigente sin imagen.`);
  process.exit(1);
}
console.log(`OK: imágenes de publicación vigentes validadas. Avisos históricos: ${warnings}.`);
